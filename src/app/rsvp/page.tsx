    ...
    for (const inv of invitees) {
      if (hasRsvpd) {
        const attendingIds = new Set(
          (inv.attendingEvents ?? []).map((e) => e.id),
        );
        map[event.id][inv.id] = attendingIds.has(event.id);
      } else {
        map[event.id][inv.id] = false;
      }
    }
  }
  return map;
...
function RSVPForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const inviteeTxns = invitees.flatMap((inv) => {
        ...
        return [
          ...(toUnlink.length
            ? [db.tx.invitees[inv.id].unlink({ attendingEvents: toUnlink })]
            : []),
          ...(toLink.length
            ? [db.tx.invitees[inv.id].link({ attendingEvents: toLink })]
            : []),
        ];
      });
      await db.transact([
  ...
  if (rsvpLocked && !hasRsvpd) {
    return (
      ...
      >
        ...
        >
          RSVP closed on {RSVP_DEADLINE_DISPLAY}. Please reach out to the couple
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      {!rsvpLocked && (
        <p
          className="mb-4 text-right text-sm transition-opacity duration-300"
          style={{
      ...
      {!rsvpLocked && (
        <div
          className="mt-10 rounded-lg px-6 py-5"
          style={{
            backgroundColor: "var(--color-surface)",
            border: "1px solid var(--color-border)",
          }}
    ...
    >
      <div className="mx-auto max-w-2xl px-5 pt-32">
        ...
        <div
          ...
          style={{ backgroundColor: "var(--color-gold-dim)", opacity: 0.5 }}
        />
      </div>
    </div>
  );
